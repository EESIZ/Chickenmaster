...
class StoryContext:
    ...
    def get_metric_correlation(self, metric1: MetricEnum, metric2: MetricEnum, days: int = 7) -> float:
        """
        두 지표 간의 상관관계를 계산합니다.
        ...
        if len(trend1) != len(trend2) or len(trend1) < 2:
            return 0.0
        ...
        # 공분산 계산
        covariance = sum((x - mean1) * (y - mean2) for x, y in zip(trend1, trend2)) / len(trend1)
