...
class EventGenerator:
    def __init__(self) -> None:
        ...
        if not self.api_key:
            raise ValueError(
                "API 키가 설정되지 않았습니다. config.py 실행하여 확인하세요."
            )
        # Anthropic 클라이언트 초기화
        ...
        try:
            ...
            raise ImportError(
                "anthropic 패키지가 설치되지 않았습니다. pip install anthropic 명령으로 설치하세요."
            )

    def generate_events(
        self, category: str, count: int, seed: int = 42, variants: bool = False
    ) -> List[Dict[str, Any]]:
        """
        이벤트 생성 메인 메서드
...
def main() -> int:
    ...
    parser.add_argument(
        "--n", type=int, default=10, help="생성할 이벤트 수 (기본값: 10)"
    )
    ...
    try:
        ...
        events = generator.generate_events(
            category=args.category, count=args.n, seed=args.seed, variants=args.variants
        )
