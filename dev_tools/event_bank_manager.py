...
class EventBankManager:
    ...
    def __init__(self) -> None:
        ...
        self.events: dict[str, list[dict[str, Any]]] = {category: [] for category in self.CATEGORIES}
...
def main() -> int:
    ...
    parser.add_argument(
        "--load", action="store_true", help="모든 이벤트 로드"
    )
    parser.add_argument(
        "--validate", action="store_true", help="모든 이벤트 검증"
    )
    parser.add_argument(
        "--metrics", action="store_true", help="품질 메트릭 계산"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--export", type=str, help="이벤트 뱅크를 JSON 파일로 내보내기"
    )
    parser.add_argument(
        "--backup", action="store_true", help="이벤트 뱅크 백업"
    )
    parser.add_argument(
        "--stats", action="store_true", help="이벤트 뱅크 통계 출력"
    )
    parser.add_argument(
    ...
    # 기본 동작: 모든 이벤트 로드
    if not any([args.load, args.validate, args.metrics, args.simulate, 
                args.export, args.backup, args.stats]):
