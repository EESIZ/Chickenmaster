...
class EventBankIndexer:
    ...
    def integrate_events(self, events: List[Dict[str, Any]], metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for event in events:
            ...
            # 이벤트 ID 확인
            event_id = event.get("id", f"{category}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{integrated_count}")
    ...
    def update_metrics(self, metadata: Dict[str, Any], metrics: Dict[str, float]) -> Dict[str, Any]:
        """
        품질 메트릭 업데이트
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="이벤트 뱅크 메타데이터 인덱싱 및 관리 도구")
    parser.add_argument("--input", "-i", required=True, help="입력 이벤트 JSON 파일 경로")
    parser.add_argument("--output-dir", "-o", default="data/events", help="이벤트 뱅크 루트 디렉토리 경로")
    parser.add_argument("--diversity", type=float, help="다양성 점수 (0.0-1.0)")
    parser.add_argument("--tradeoff", type=float, help="트레이드오프 명확성 점수 (0.0-1.0)")
    parser.add_argument("--authenticity", type=float, help="문화적 진정성 점수 (0.0-1.0)")
    parser.add_argument("--replayability", type=float, help="재플레이 가치 점수 (0.0-1.0)")
