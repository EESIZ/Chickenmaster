    ...
    def save_events(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️ 오류 발생으로 빈 이벤트 파일이 생성되었습니다: {self.output_file}")
            ...
            if "trigger" in event:
                if "condition" in event["trigger"]:
                    ...
                    if original_condition in self.CONDITION_MAPPING:
                        event["trigger"]["condition"] = self.CONDITION_MAPPING[original_condition]
                        fixed_count += 1
                        print(f"✅ 이벤트 {event['id']}의 트리거 조건 수정: {original_condition} → {event['trigger']['condition']}")
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="LLM 생성 이벤트의 트리거 조건을 자동으로 교정하는 도구")
    parser.add_argument("--input", "-i", required=True, help="입력 이벤트 JSON 파일 경로")
    parser.add_argument("--output", "-o", help="출력 이벤트 JSON 파일 경로 (기본값: 입력 파일명_fixed.json)")
