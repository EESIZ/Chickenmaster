    ...
    for doc_path in all_docs:
        ...
        for text, link in markdown_links:
            ...
            if not target_path.exists():
                broken_links.append(
                    {"file": doc_path.relative_to(PROJECT_ROOT), "link": link, "text": text}
                )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                stats["recent_updates"].append(
                    {"file": doc_path.relative_to(PROJECT_ROOT), "last_modified": last_modified}
                )
        except Exception:
            pass  # Git 정보 없음
...
def update_adr_index():
    ...
    # 모든 ADR 파일 찾기
    adr_files = sorted(
        [f for f in ADR_DIR.glob("*.md") if f.name not in {"README.md", "template.md"}]
    )
