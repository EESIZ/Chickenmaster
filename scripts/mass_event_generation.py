...
class MassEventGenerator:
    ...
    def generate_single_event(self, category: str, tags: List[str], attempt: int = 1) -> Dict[str, Any] | None:
        """단일 이벤트 생성 (재시도 로직 포함)"""
        max_attempts = 3
        ...
        for attempt_num in range(max_attempts):
            try:
                ...
                if self.validator.validate_event(event):
                    ...
                    print(f"❌ 검증 실패 (시도 {attempt_num + 1}): {', '.join(self.validator.errors)}")
    ...
    def generate_category_events(self, category: str, target_count: int, tags: List[str]) -> List[Dict[str, Any]]:
        """카테고리별 이벤트 대량 생성"""
    ...
    def save_events(self, events: List[Dict[str, Any]], category: str, intermediate: bool = False) -> str:
        """이벤트 저장"""
        if not events:
            return ""
    ...
    def run_mass_generation(self) -> Dict[str, int]:
        ...
        print(f"📈 전체 성공률: {total_generated}/{total_target} ({total_generated/total_target*100:.1f}%)")
