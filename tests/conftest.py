...
from src.events.validators.specific import FormulaValidator
...
def mock_event_container(mock_event: Event) -> EventContainer[Event]:
    ...
    return EventContainer[Event](events=[mock_event], metadata={"test": True})
...
def invalid_validation_result() -> ValidationResult:
    ...
    return ValidationResult(is_valid=False, errors=["테스트 오류"], warnings=["테스트 경고"])
