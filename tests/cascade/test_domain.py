...
class TestTriggerCondition:
    ...
    def test_create_with_parameters(self):
        ...
        condition = TriggerCondition(
            expression="metrics.money > {threshold} and {is_active}", parameters=params
        )
...
class TestPendingEvent:
    ...
    def test_create_pending_event(self):
        """기본 생성 테스트."""
        event = PendingEvent(event_id="test_event", delay_turns=2, trigger_turn=5)
    ...
    def test_invalid_delay(self):
        """유효하지 않은 지연 턴 테스트."""
        with pytest.raises(ValueError):
            PendingEvent(event_id="test_event", delay_turns=0, trigger_turn=5)

    def test_invalid_probability(self):
        """유효하지 않은 확률 테스트."""
        with pytest.raises(ValueError):
            PendingEvent(event_id="test_event", delay_turns=2, trigger_turn=5, probability=1.5)
...
class TestCascadeNode:
    ...
    def test_create_root_node(self):
        """루트 노드 생성 테스트."""
        node = CascadeNode(event_id="root_event", depth=0, cascade_type=CascadeType.IMMEDIATE)
    ...
    def test_invalid_depth(self):
        """유효하지 않은 깊이 테스트."""
        with pytest.raises(ValueError):
            CascadeNode(event_id="test_event", depth=-1, cascade_type=CascadeType.IMMEDIATE)
    ...
    def test_delayed_without_delay(self):
        ...
        with pytest.raises(ValueError):
            CascadeNode(
                event_id="test_event", depth=0, cascade_type=CascadeType.DELAYED, delay_turns=0
            )

    def test_conditional_without_condition(self):
        """조건 없는 조건부 이벤트 테스트."""
        with pytest.raises(ValueError):
            CascadeNode(event_id="test_event", depth=0, cascade_type=CascadeType.CONDITIONAL)
...
class TestCascadeChain:
    ...
    def test_create_simple_chain(self):
        """단순 체인 생성 테스트."""
        root_node = CascadeNode(event_id="root_event", depth=0, cascade_type=CascadeType.IMMEDIATE)
        ...
        chain = CascadeChain(root_event_id="root_event", nodes=frozenset([root_node, child_node]))
    ...
    def test_empty_nodes(self):
        """노드 없는 체인 테스트."""
        with pytest.raises(ValueError):
            CascadeChain(root_event_id="root_event", nodes=frozenset())
    ...
    def test_invalid_max_depth(self):
        ...
        with pytest.raises(ValueError):
            CascadeChain(root_event_id="root_event", nodes=frozenset([root_node]), max_depth=0)
    ...
    def test_missing_root_node(self):
        ...
        with pytest.raises(ValueError):
            CascadeChain(root_event_id="root_event", nodes=frozenset([child_node]))

    def test_get_nodes_at_depth(self):
        """특정 깊이 노드 조회 테스트."""
        root_node = CascadeNode(event_id="root_event", depth=0, cascade_type=CascadeType.IMMEDIATE)
        child1 = CascadeNode(
            event_id="child1", parent_id="root_event", depth=1, cascade_type=CascadeType.IMMEDIATE
        )
        child2 = CascadeNode(
            event_id="child2", parent_id="root_event", depth=1, cascade_type=CascadeType.IMMEDIATE
        )

        chain = CascadeChain(
            root_event_id="root_event", nodes=frozenset([root_node, child1, child2])
        )
    ...
    def test_get_child_nodes(self):
        ...
        grandchild = CascadeNode(
            event_id="grandchild", parent_id="child1", depth=2, cascade_type=CascadeType.IMMEDIATE
        )

        chain = CascadeChain(
            root_event_id="root_event", nodes=frozenset([root_node, child1, child2, grandchild])
        )
    ...
    def test_get_node_by_event_id(self):
        ...
        root_node = CascadeNode(event_id="root_event", depth=0, cascade_type=CascadeType.IMMEDIATE)
        child = CascadeNode(
            event_id="child", parent_id="root_event", depth=1, cascade_type=CascadeType.IMMEDIATE
        )

        chain = CascadeChain(root_event_id="root_event", nodes=frozenset([root_node, child]))
    ...
    def test_get_max_actual_depth(self):
        ...
        grandchild = CascadeNode(
            event_id="grandchild", parent_id="child", depth=2, cascade_type=CascadeType.IMMEDIATE
        )

        chain1 = CascadeChain(root_event_id="root_event", nodes=frozenset([root_node]))
        chain2 = CascadeChain(root_event_id="root_event", nodes=frozenset([root_node, child]))
        chain3 = CascadeChain(
            root_event_id="root_event", nodes=frozenset([root_node, child, grandchild])
        )
...
class TestCascadeResult:
    ...
    def test_invalid_depth(self):
        ...
        with pytest.raises(ValueError):
            CascadeResult(
                triggered_events=(), pending_events=(), metrics_impact={}, depth_reached=-1
            )

    def test_get_total_events_count(self):
        """총 이벤트 수 계산 테스트."""
        pending1 = PendingEvent(event_id="pending1", delay_turns=1, trigger_turn=5)
        pending2 = PendingEvent(event_id="pending2", delay_turns=2, trigger_turn=6)
    ...
    def test_get_total_metrics_impact(self):
        ...
        assert (
            result.get_total_metrics_impact() == MAGIC_NUMBER_ONE_HUNDRED_FIFTEEN
        )  # |-100| + |5| + |-10| = 115

    def test_has_pending_events(self):
        """지연 이벤트 여부 테스트."""
        result1 = CascadeResult(
            triggered_events=("event1",), pending_events=(), metrics_impact={}, depth_reached=1
        )

        pending = PendingEvent(event_id="pending", delay_turns=1, trigger_turn=5)
