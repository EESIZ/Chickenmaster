...
class MockEventService(IEventService):
    ...
    def __init__(self):
        self.events = {
            "root_event": TestEvent(id="root_event", effects=[{"metric": "money", "value": -100}]),
            "child1": TestEvent(id="child1", effects=[{"metric": "reputation", "value": 5}]),
            "child2": TestEvent(id="child2", effects=[{"metric": "happiness", "value": -10}]),
            "grandchild": TestEvent(id="grandchild", effects=[{"metric": "money", "value": 50}]),
            "conditional_event": TestEvent(
                id="conditional_event", effects=[{"metric": "reputation", "value": 20}]
            ),
            "probabilistic_event": TestEvent(
                id="probabilistic_event", effects=[{"metric": "happiness", "value": 15}]
            ),
            "delayed_event": TestEvent(
                id="delayed_event", effects=[{"metric": "money", "value": 200}]
            ),
        }
        self.cooldowns = {}
...
class TestCascadeServiceImpl:
    ...
    def game_state(self):
        """게임 상태 fixture."""
        return TestGameState(metrics={"money": 1000, "reputation": 50, "happiness": 75})

    def test_register_cascade_relation(self, cascade_service):
        """연쇄 관계 등록 테스트."""
        node = cascade_service.register_cascade_relation(
            parent_event_id="root_event", child_event_id="child1", cascade_type_str="IMMEDIATE"
        )
        ...
        assert (
            node.depth == 0
        )  # 첫 번째 등록이므로 깊이는 0 + 1 = 1이어야 하지만, 구현상 0으로 시작

    def test_register_conditional_relation(self, cascade_service):
        """조건부 연쇄 관계 등록 테스트."""
        condition = {"expression": "metrics.money > 1000", "parameters": {"threshold": 1000}}
    ...
    def test_build_cascade_chain(self, cascade_service):
        ...
        cascade_service.register_cascade_relation(
            parent_event_id="root_event", child_event_id="child2", cascade_type_str="IMMEDIATE"
        )
        cascade_service.register_cascade_relation(
            parent_event_id="child1", child_event_id="grandchild", cascade_type_str="IMMEDIATE"
        )
        ...
        assert (
            chain.get_max_actual_depth() == MAGIC_NUMBER_TWO
        )  # 최대 깊이는 2 (루트=0, 자식=1, 손자=2)
    ...
    def test_process_cascade_chain(self, cascade_service, event_service, game_state):
        ...
        result = cascade_service.process_cascade_chain(
            root_event=root_event, game_state=game_state, current_turn=1
        )
    ...
    def test_calculate_metrics_impact(self, cascade_service, event_service):
        ...
        # 게임 상태
        game_state = TestGameState(metrics={"money": 1000, "reputation": 50, "happiness": 75})
