...
def sample_story_context():
    ...
    metrics_history = [
        MetricsHistory(
            day=1, metrics={"money": 10000, "reputation": 50, "happiness": 60, "pain": 20}
        ),
        MetricsHistory(
            day=2, metrics={"money": 9500, "reputation": 55, "happiness": 55, "pain": 25}
        ),
        MetricsHistory(
            day=3, metrics={"money": 9000, "reputation": 60, "happiness": 50, "pain": 30}
        ),
    ]
...
class TestGetStoryPatterns:
    ...
    def test_tradeoff_pattern_matching(self, storyteller_service, sample_story_context):
        ...
        modified_history = sample_story_context.metrics_history[:-1] + [
            MetricsHistory(
                day=3, metrics={"money": 4000, "reputation": 25, "happiness": 40, "pain": 50}
            )
        ]
        ...
        financial_pattern = next(
            (p for p in tradeoff_patterns if "financial" in p.pattern_id), None
        )
    ...
    def test_no_right_answer_pattern_late_game(self, storyteller_service, sample_story_context):
        ...
        sample_story_context.metrics_history[-1] = MetricsHistory(
            day=3, metrics={"money": 15000, "reputation": 75, "happiness": 60, "pain": 45}
        )
    ...
    def test_empty_metrics_history_raises_error(self, storyteller_service):
        ...
        invalid_context = StoryContext(
            day=1, game_progression=0.1, metrics_history=[], recent_events=[]  # 빈 히스토리
        )
...
class TestAnalyzeMetricsTrend:
    ...
    def test_insufficient_history_raises_error(self, storyteller_service):
        ...
        insufficient_context = StoryContext(
            ...
            metrics_history=[MetricsHistory(day=1, metrics={"money": 10000})],  # 단일 히스토리
            recent_events=[],
        )
...
class TestSuggestEvent:
    ...
    def test_tradeoff_aware_event_suggestion(
        ...
        # 자금 부족 상황 설정
        sample_story_context.metrics_history[-1] = MetricsHistory(
            day=3, metrics={"money": 3000, "reputation": 60, "happiness": 50, "pain": 30}
        )
...
class TestGenerateNarrative:
    ...
    def test_no_right_answer_narrative_philosophy(
        ...
        # 복잡한 상황 설정
        sample_story_context.metrics_history[-1] = MetricsHistory(
            day=3, metrics={"money": 8000, "reputation": 45, "happiness": 55, "pain": 40}
        )
...
class TestPrivateHelperMethods:
    ...
    def test_critical_metrics_identification_with_tradeoff(self, storyteller_service):
    ...
    def test_pattern_prioritization_by_game_progression(self, storyteller_service):
        ...
        early_patterns = storyteller_service._prioritize_patterns_by_progression(patterns, 0.2)
        assert early_patterns[0].pattern_type == "uncertainty"

        # 중기 게임 (tradeoff 우선)
        mid_patterns = storyteller_service._prioritize_patterns_by_progression(patterns, 0.5)
        assert mid_patterns[0].pattern_type == "tradeoff"

        # 후기 게임 (noRightAnswer 우선)
        late_patterns = storyteller_service._prioritize_patterns_by_progression(patterns, 0.8)
