...
class TestCascadeServiceImpl(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.event_service.get_event_by_id = MagicMock(side_effect=lambda id: {
            "event1": self.event1,
            "event2": self.event2,
            "event3": self.event3
        }.get(id))
        
        self.event_service.apply_event = MagicMock(
            side_effect=lambda event, game_state: game_state
        )
