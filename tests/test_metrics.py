...
def test_max_snapshots_limit(test_metrics: dict[MetricEnum, float], temp_data_dir: str) -> None:
    ...
    # 스냅샷 파일 개수 확인
    snapshot_files = [
        f for f in os.listdir(temp_data_dir) if f.startswith("metrics_snap_")
    ]
    assert len(snapshot_files) <= SNAPSHOT_LIMIT, f"스냅샷 개수가 제한을 초과함: {len(snapshot_files)}"
...
def test_concurrent_metric_updates(test_metrics: dict[MetricEnum, float]) -> None:
    ...
    happiness_suffering_sum = result_metrics[MetricEnum.HAPPINESS] + result_metrics[MetricEnum.SUFFERING]
