...
class Metric:
    ...
    def with_new_value(self, new_value: int) -> "Metric":
        ...
        return Metric(
            name=self.name, value=clamped_value, min_value=self.min_value, max_value=self.max_value
        )
...
class TradeoffPair:
    ...
    def calculate_effect(self, primary_delta: int, primary_metric: str) -> dict[str, int]:
        ...
        return {primary_metric: primary_delta, secondary_metric: secondary_delta}
...
class MetricsSnapshot:
    ...
    def get_metric_value(self, metric_name: str) -> int:
        ...
        return self.metrics.get(
            metric_name, Metric(name=metric_name, value=0, min_value=0, max_value=100)
        ).value
    ...
    def with_updated_metric(self, metric_name: str, new_value: int) -> "MetricsSnapshot":
        ...
        if metric_name in updated_metrics:
            ...
            updated_metrics[metric_name] = Metric(
                name=metric_name, value=new_value, min_value=0, max_value=100
            )

        return MetricsSnapshot(metrics=updated_metrics, timestamp=self.timestamp)
