...
class IMetricService(ABC):
    ...
    def calculate_happiness_pain_balance(
        self, current_happiness: int, current_pain: int, effects: dict[str, int]
    ) -> tuple[int, int]:
    ...
    @abstractmethod
    def calculate_game_over_conditions(self, game_state: GameState) -> dict[str, bool]:
        """게임 종료 조건들 검증
    ...
    def apply_tradeoff_effects(
        self, primary_effect: dict[str, int], metrics_snapshot: MetricsSnapshot
    ) -> dict[str, int]:
        """트레이드오프 효과 적용
    ...
    @abstractmethod
    def get_critical_metrics(self, game_state: GameState) -> dict[str, Metric]:
        """위험 수준의 지표 목록 반환
