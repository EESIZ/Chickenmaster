...
class TriggerCondition:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.parameters and "{" in self.expression:
            raise ValueError(
                f"조건식 '{self.expression}'에 파라미터가 필요하지만 제공되지 않았습니다."
            )
...
class CascadeChain:
    ...
    def __post_init__(self) -> None:
        ...
        root_exists = any(
            node.event_id == self.root_event_id and node.is_root() for node in self.nodes
        )
        if not root_exists:
            raise ValueError(
                f"루트 이벤트 ID '{self.root_event_id}'에 해당하는 루트 노드가 없습니다."
            )
