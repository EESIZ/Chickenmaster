...
def tradeoff_apply_price_change(price_change: float, metrics: Dict[Metric, float], 
                               config: Optional[Dict[str, Any]] = None) -> Dict[Metric, float]:
    """
...
def update_economy_state(current_state: Dict[str, Any], decision: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if Metric.INVENTORY in updated_metrics:
        updated_metrics[Metric.INVENTORY] = cap_metric_value(Metric.INVENTORY, updated_metrics[Metric.INVENTORY])
    
    if Metric.MONEY in updated_metrics:
        updated_metrics[Metric.MONEY] = cap_metric_value(Metric.MONEY, updated_metrics[Metric.MONEY])
